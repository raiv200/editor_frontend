...
export default function RfpEditorPage({ params }: PageProps) {
    ...
    const fetchRfp = async () => {
      try {
        ...
        if (data.sections?.length) {
          ...
          // Track which questions have answers (check both answer content and answeredAt)
  ...
  useEffect(() => {
    ...
    const fetchAnswer = async () => {
      ...
      try {
        ...
        // Update answered status - only if there's actual content
  ...
  const handleSaveAnswer = useCallback(
    async (content: { html: string; json: object }) => {
      ...
      // Mark as answered only if there's content
              ...
              <div className="max-w-4xl mx-auto">
                ...
                {collabToken ? (
                  loadingAnswer ? (
                    ...
                    <CollaborativeEditor
                      // IMPORTANT: Use question ID in key to force remount when switching questions
                      // This ensures proper cleanup and new connection for each question's document
